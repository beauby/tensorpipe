...
TEST_P(TransportTest, Connection_Initialization) {
  ...
  this->testConnection(
      [&](std::shared_ptr<Connection> conn) {
        this->doRead(conn, [&, conn](const Error& error,
                                     const void* /* unused */, size_t len) {
...
TEST_P(TransportTest, Connection_InitializationError) {
  ...
  this->testConnection(
      ...
      [&](std::shared_ptr<Connection> conn) {
        this->doRead(conn,
                     [&, conn](const Error& error, const void* /* unused */,
                               size_t /* unused */) {
                       ASSERT_TRUE(error);
                       readCompletedProm.set_value();
                     });
...
TEST_P(TransportTest, DISABLED_Connection_DestroyConnectionFromCallback) {
  this->testConnection(
      ...
      [&](std::shared_ptr<Connection> conn) {
        ...
        this->doRead(conn,
                     [conn](const Error& /* unused */, const void* /* unused */,
...
TEST_P(TransportTest, Connection_QueueWritesBeforeReads) {
  ...
  this->testConnection(
      [&](std::shared_ptr<Connection> conn) {
        for (int i = 0; i < numMsg; i++) {
          this->doWrite(conn, msg[i].c_str(), msg[i].length(),
      ...
      [&](std::shared_ptr<Connection> conn) {
        ...
        for (int i = 0; i < numMsg; i++) {
          this->doRead(conn, [&, conn, i](const Error& error, const void* data,
                                          size_t len) {
            ASSERT_FALSE(error) << error.what();
