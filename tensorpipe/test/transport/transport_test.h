 ...
 public:
  ...
  class Bomb {
   public:
    ...
    void defuse() { defused_ = true; }

    ~Bomb() { EXPECT_TRUE(defused_); }

   private:
    bool defused_ = false;
  };

  std::shared_ptr<Bomb> armBomb() { return std::make_shared<Bomb>(); }

  void doRead(std::shared_ptr<tensorpipe::transport::Connection> conn,
  ...
  void doWrite(std::shared_ptr<tensorpipe::transport::Connection> conn,
    ...
    // that we deadlock if the callback is invoked inline.
    std::lock_guard<std::mutex> outerLock(*mutex);
    conn->write(ptr, length,
                [fn{std::move(fn)}, mutex,
                 bomb{armBomb()}](const tensorpipe::Error& error) {
                  std::lock_guard<std::mutex> innerLock(*mutex);
                  bomb->defuse();
                  fn(error);
                });
