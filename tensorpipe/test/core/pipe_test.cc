...
class SimpleWriteReadTest : public ClientServerPipeTestCase {
  InlineMessage imessage_ = {
      ...
      .tensors =
          {
              {.data = "tensor #1", .metadata = "tensor metadata #1"},
              {.data = "tensor #2", .metadata = "tensor metadata #2"},
              {.data = "tensor #3", .metadata = "tensor metadata #3"},
          },
 ...
 public:
  ...
  void client(Pipe& pipe) override {
    ...
    Storage storage;
    auto future = pipeReadWithFuture(pipe);
...
class MultipleWriteReadTest : public ClientServerPipeTestCase {
  InlineMessage imessage1_ = {
      ...
      .tensors =
          {
              {.data = "tensor #1.1", .metadata = "tensor metadata #1.1"},
          },
  ...
  InlineMessage imessage2_ = {
      ...
      .tensors =
          {
              {.data = "tensor #2.1", .metadata = "tensor metadata #2.1"},
          },
      .metadata = "message metadata",
  };
 ...
 public:
  ...
  void client(Pipe& pipe) override {
    auto future1 = pipeReadWithFuture(pipe);
    auto future2 = pipeReadWithFuture(pipe);
